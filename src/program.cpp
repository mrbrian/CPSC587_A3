...
void Program::loadQuadGeometryToGPU() {
...
void Program::reloadProjectionMatrix() {
	// Perspective Only
...
void Program::loadModelViewMatrix() {
	M = IdentityMatrix();
	// view doesn't change, but if it did you would use this
	V = camera.lookatMatrix();
}

void Program::reloadViewMatrix() { V = camera.lookatMatrix(); }

void Program::setupModelViewProjectionTransform() {
	MVP = P * V * M; // transforms vertices from right to left (odd huh?)
}

void Program::reloadMVPUniform() {
	Mat4f MV = V * M;
...
void Program::reloadColorUniform(float r, float g, float b) {
